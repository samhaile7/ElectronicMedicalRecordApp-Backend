...
public class RangeCheckerService {
	...
	private boolean valuesWithinRange(Patient patient) {
		boolean output = true;
		if (patient.getDiastolicBloodPressure() > MAX_DIASTOLIC_PRESSURE
				|| patient.getDiastolicBloodPressure() < MIN_DIASTOLIC_PRESSURE) {
			output = false;
		}
		;
		if (patient.getSystolicBloodPressure() > MAX_SYSTOLIC_PRESSURE
				|| patient.getSystolicBloodPressure() < MIN_SYSTOLIC_PRESSURE) {
			output = false;
		}
		;
		if (patient.getPartialThromboplastinTime() > MAX_PARTIAL_THROMBOPLASTIN_TIME
				|| patient.getPartialThromboplastinTime() < MIN_PARTIAL_THROMBOPLASTIN_TIME) {
			output = false;
		}
		;
		...
		;
		if (patient.getRespirationRate() > MAX_RESPIRATION_RATE
				|| patient.getRespirationRate() < MIN_RESPIRATION_RATE) {
			output = false;
		}
		;
