...
public class RangeCheckerService {
    ...
    final Long CLEAR_STRING_ID = 1L;
    final Long NOT_CLEAR_ID = 2L;
    ...
    public Patient assignPatientMobilityStatus(Patient patient) {
        if (valuesWithinRange(patient)) {
            patient.setMobilityStatus(mobilityStatusRepository.findByMobilityStatusId(CLEAR_STRING_ID));
        }
        else {
            patient.setMobilityStatus(mobilityStatusRepository.findByMobilityStatusId(NOT_CLEAR_ID));
        }
        return patient;
